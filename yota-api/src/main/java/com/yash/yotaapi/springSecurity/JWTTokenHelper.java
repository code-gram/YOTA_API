 ...
 * 
 * @Component public class JWTTokenHelper { private static final long
 * JWT_TOKEN_VALIDITY = 5 * 60 * 60; //we have mention token expire time private
 * String SECRET_KEY = "secret";
 * 
 * public String getUserNameFromToken(String token) { return
 * getClaimFronToken(token, Claims::getSubject); }
 * 
 * public Date getExpirationDateFromToken(String token) { return
 * getClaimFronToken(token, Claims::getExpiration); }
 * 
 * public <T> T getClaimFronToken(String token, Function<Claims, T>
 * claimsResolver) { final Claims claims = getAllClaimsFromToekn(token); return
 * claimsResolver.apply(claims); }
 * 
 * private Claims getAllClaimsFromToekn(String token) { return
