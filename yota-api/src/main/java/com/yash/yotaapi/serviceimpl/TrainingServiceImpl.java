...
//        }
    ...
    /**
     * @apiNote Trainer name(not username) and email id must exist in order to send the mail successfully
     */
    @Override
    public Training updateActualStartAndEndDate(Training training, long trainingId) {
        ...
        if (ObjectUtils.isNotEmpty(updatedTraining)) {
            ...
            MailRequest mailRequest = MailRequest.builder()
                    .to(Collections.singletonList(trainerUserDetails.getEmailId()))
                    .subject("Training Request Approved")
                    .body("Hi, <b>"
                            + trainerUserDetails.getName()
                            + "</b>, You have been assigned a new Training: <b>"
                            + updatedTraining.getTrainingName()
                            + "</b> with <b>Start Date: "
                            + updatedTraining.getActualStartDate()
                            + " and End Date: "
